(! rsum-code
    ...
    (pushl %ebp)
    (rrmovl %esp %ebp)
    (mrmovl 8 (%ebp) %edx) ;get address of ele1
    (andl %edx %edx) ;if at adddres 0 we dont do siht
    (je End)
    (mrmovl 0 (%edx) %ecx)
    (addl %ecx %eax)
    (mrmovl 4 (%edx) %ecx) ;address of next node
    (pushl %ecx)
    (call rsum-list)
...
(! symbol-table
   (hons-shrink-alist
    (y86-symbol-table (@ rsum-code) (@ location) 'symbol-table)
    'shrunk-symbol-table))
...
(!! init-mem
    (hons-shrink-alist
     (y86-asm (@ rsum-code) (@ location) (@ symbol-table) 'fib-iterative)
